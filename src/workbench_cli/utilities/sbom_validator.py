...
class SBOMValidator:
    ...
    def _detect_sbom_format(file_path: str) -> str:
        ...
        spdx_markers = [
            '"spdxVersion"', '"spdxversion"',  # JSON format
            '"SPDXID"', '"spdxid"',  # JSON format
            'spdx:Document', 'spdx:document',  # RDF format
            '<spdx:', '<SPDX:',  # XML format
            'SPDXVersion:', 'spdxversion:',  # Tag-value format
        ]
    ...
    def _prepare_spdx_for_upload(file_path: str, document: Any) -> str:
        ...
        if file_ext == '.json':
            ...
            try:
                ...
                raise ValidationError("SPDX tools library not available for conversion. Please install spdx-tools.") from e
