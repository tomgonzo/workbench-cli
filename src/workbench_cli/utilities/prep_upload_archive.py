...
class UploadArchivePrep:
            ...
            with zipfile.ZipFile(archive_path, 'w', zipfile.ZIP_DEFLATED, compresslevel=6) as zipf:
                ...
                for root, dirs, files in os.walk(abs_path):
                    ...
                    for d in dirs:
                        ...
                        if has_gitignore:
                            relative_dir_path = os.path.join(normalized_rel_root, d) if normalized_rel_root else d
                            if UploadArchivePrep._is_excluded_by_gitignore(relative_dir_path, gitignore_patterns, is_dir=True):
                                dirs_to_remove.append(d)
                                files_excluded += 1
                    ...
                    for file in files:
                        ...
                        # Check gitignore patterns
                        if has_gitignore and UploadArchivePrep._is_excluded_by_gitignore(rel_file_path, gitignore_patterns):
                            files_excluded += 1
                        ...
                        if not UploadArchivePrep.validate_file_for_archive(file_path):
                            files_excluded += 1
                            logger.warning(f"Skipped invalid file: {rel_file_path} (type: {UploadArchivePrep._get_file_type_description(file_path)})")
                            continue
                ...
                print(f"Added {files_added} files to archive (excluded {files_excluded} files/directories)")
    ...
    @staticmethod
    def _is_excluded_by_gitignore(path: str, gitignore_patterns: List[str], is_dir: bool = False) -> bool:
        """
