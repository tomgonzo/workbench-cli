...
def _validate_sbom_file(file_path: str) -> Tuple[str, str, Dict]:
    """
    ...
    Returns:
        tuple[str, str, Dict]: (format, version, metadata)
    ...
    """
    try:
        sbom_format, version, metadata = SBOMValidator.validate_sbom_file(file_path)
        logger.debug(f"SBOM validation successful: {sbom_format} v{version}")
        return sbom_format, version, metadata
...
def handle_import_sbom(workbench: "WorkbenchAPI", params: argparse.Namespace) -> bool:
    ...
    try:
        sbom_format, version, metadata = _validate_sbom_file(params.path)
    ...
    # Resolve project and scan (find or create) - AFTER validation
    ...
    print("\n--- Uploading SBOM File ---")
    try:
        workbench.upload_sbom_file(scan_code=scan_code, path=params.path)
        print(f"SBOM file uploaded successfully from: {params.path}")
    except Exception as e:
